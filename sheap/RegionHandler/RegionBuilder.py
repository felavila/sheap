...
class RegionBuilder:
    ...
    def __init__(self,xmin,xmax,n_narrow=1,n_broad=1):
        ...
        self.tied_pairsv2 = [(["OIIIb","OIIIc","1"],["amplitude_OIIIb_1_narrow", "amplitude_OIIIc_1_narrow", "*0.3"]),
                        (["NIIb","NIIa","1"],["amplitude_NIIb_1_narrow", "amplitude_NIIa_1_narrow", "*0.3"])]
    ...
    def narrow_lines_handler(self,values,n_narrow,add_out_flow):
        ...
        for n in range(n_narrow):
            ...
            if add_out_flow and n==0:
                ...
                local_copy.update({"center":local_copy["center"]-5,"kind": "outflow","component": 10}) #the idea is this never get attached or it is?
                local_.append(local_copy)
        return local_
    ...
    def make_region(self,xmin=None,xmax=None,n_broad=None,n_narrow=None, 
                ...
                ,narrow_plus= False,verbose= False,tied_to= []
                ,force_linear= False,add_out_flow= False
        ...
        for key,region in self.full_regions.items(): #full_regions this should already have reduce the regions?
            for values in  region['region']:  
                ...
                if key in main_regions and xmin <= values["center"] <= xmax:
                    ...
                    local_ = self.broad_regions_handler(values,n_narrow)
        ...
        if local_region_list:
            for pair,factor in self.tied_pairsv2:
        ...
        params = ["center","width"]
        self.tied_params += [[f"{p}_{line['line_name']}_{line["component"]}_{line['kind']}", f"{p}_{tied_narrow_to}_{line["component"]}_narrow"] for line in local_region_list if (line['kind'] == "narrow" and line["line_name"] != tied_narrow_to) for p in params]
        
        self.tied_params += [[f"{p}_{line['line_name']}_{line["component"]}_{line['kind']}", f"{p}_{tied_broad_to}_{line["component"]}_broad"] for line in local_region_list if (line['kind'] == "broad" and line["line_name"] != tied_broad_to) for p in params]       
