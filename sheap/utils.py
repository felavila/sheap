...
def prepare_spectra(spectra_list,outer_limits):
    ...
    spectral_region, _,_,mask_region = mask_builder(spectra_reshaped,outer_limits=outer_limits)
...
@jit
def mask_builder(sheap_array, inner_limits=[0,0], outer_limits=None,instrumental_limit= 10e50):
    ...
    mask = (sheap_array[:,0,:] >= inner_limits[0]) & (sheap_array[:,0,:] <= inner_limits[1])  
    if outer_limits is not None:
        mask_outside_outter = (sheap_array[:, 0, :] < outer_limits[0]) | (sheap_array[:, 0, :] > outer_limits[1])
        mask = mask | mask_outside_outter 
    mask = mask | (jnp.isnan(sheap_array[:,0,:]) | jnp.isinf(sheap_array[:, 2, :])) | (sheap_array[:,1,:]<=0)
...
@jit
def prepare_uncertainties(y_uncertainties: Optional[jnp.ndarray], y_data: jnp.ndarray) -> jnp.ndarray:
    """
...
def resize_and_fill_with_nans(original_array, new_xaxis_lenght,number_colums=3):
    ...
    slices = tuple(slice(0, min(o, t)) for o, t in zip(original_array.shape, (number_colums,new_xaxis_lenght)))
