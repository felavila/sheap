...
class Sheap_ploting:
    def __init__(self,test_clase,fit_region_g,mask_fit,mask_fit_g,masked_uncertainties_g,Master_Gaussian,params_g,Baselines,outer_limits,AN,EWfin,signal_noise_region,host_detected,host_flux):
    ...
    def plot_combined(self,n, save='',add_baseline=True,pandas=None, **kwargs):
        ...
        if not jnp.all(self.host_flux==0):
            ...
            spectros = {}
            if add_all:
                for i,key in enumerate(['Spectra redshift corrected',"host","AGN(spectra-host)"]):
                    if key=='Spectra redshift corrected':
                        ...
                        yerr = None 
                    ax1.errorbar(x_axis, y, yerr=yerr, ecolor='lightskyblue',
                            label=key, zorder=3,alpha = 0.8)
            
            else:
                ax1.errorbar(x_axis, y_spectrum, yerr=y_err, color='b', ecolor='lightskyblue',
                            label='Spectra redshift corrected', zorder=1)

        ax1.set_ylabel(r'$ f_{\lambda}$ ($\rm 10^{-17} {\rm erg\;s^{-1}\;cm^{-2}\;\AA^{-1}}$)', fontsize=40)
        ...
        if "mask" in kwargs:
            ...
            ax1.fill_between(x_axis, *ylimit, where=mask_x, color='grey', alpha=0.5, label='Mask', zorder=1)

        ax1.legend(loc='lower center', bbox_to_anchor=(0.5, 1), fancybox=True, shadow=False, ncol=4, fontsize=30)
        ax1.set_ylim(*ylimit)
        ...
        ax2.errorbar(fit_x, fit_y, yerr=fit_err, fmt='o', color='red', alpha=0.5,
        ...
        ax2.fill_between(x_axis, 0, np.nanmax(y_spectrum), where=self.mask_fit[n], color='grey', alpha=0.5,
                        label='Mask for Linear Fit', zorder=10)
        ax2.fill_between(x_axis, 0, np.nanmax(y_spectrum), where=self.mask_fit_g[n], color='green', alpha=0.5,
                        label='Mask for Gaussian Fit', zorder=10)
        ...
        if isinstance(pandas,pd.DataFrame):
            text_string = ""
            for k in ["EWfin","vel","host_detected","AN","signal_noise_region","rAGN_stars_5100","rAGN_Cont_5100","stars_Cont_5100","agn_slope"]:
                if k=="host_detected":
        ...
        if save:
            ...
            plt.show()
    #def plot_pca(self,n,save="",**kwargs):
        
    # fig, ax1 = plt.subplots(1, 1, figsize=(35, 15))
    # # Load necessary data

    # # Plot SHEAP Spectra
    # ax1.scatter(*matched_df[["EWfin","rAGN_Cont_5100"]].values.T, color='b',label='Spectra redshift corrected', zorder=1)
    # ax1.scatter(*matched_df[matched_df["index"]==False][["EWfin","rAGN_Cont_5100"]].values.T, color='r',label='Spectra redshift corrected', zorder=1)
    # ax1.set_ylabel(r'rAGN_Cont_5100', fontsize=40)
    # ax1.set_xlabel(r'EWfin', fontsize=40)
    # ax1.tick_params(which="both", length=10, width=2, labelsize=35)
...
class Sheap_pca_ploting:
    ...
    def plot(self,n,save="",**kwargs):
        ...
        # Create subplots with shared x-axis
            ...
            #continue 
        fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(35, 15),
        ...
        model_qso = jnp.nansum(self.eigenvectors[n][10:].T * 100 * self.params_linear[n][10:], axis=1)
        model_galaxy = jnp.nansum(self.eigenvectors[n][:10].T * 100 * self.params_linear[n][:10], axis=1)
        ...
        maxs = [np.nanmax(obj),np.nanmax(linear_model),np.nanmax(model_qso),np.nanmax(model_galaxy)]
        minx = [np.nanmin(obj),np.nanmin(linear_model),np.nanmin(model_qso),np.nanmin(model_galaxy)]
        ...
        ax1.fill_between(x_axis, 0, max(maxs),where=self.masked_uncertainties[n] != 1e11, color="grey", alpha=0.1,zorder=10, label="eigenvalues coverage")
        ...
        ax2.text(0.05, 0.95, rf'{(sum(jnp.where(abs(residual)<=0.4,True,False))/sum(~jnp.isnan(residual)))*100:.3f} % between abs(0.4)',
                transform=ax2.transAxes, fontsize=30, verticalalignment='top')
...
def plot_a_spectra(spectra,save=None, **kwargs):
    ...
    if n_regions > 0:
        ...
        for i, (region, start, end) in enumerate(regions_to_plot):
            ...
            if np.any(mask):
                if yerr is not None:
                    ax.errorbar(x_axis[mask], y_axis[mask], yerr=yerr[mask], fmt='-',lw=1,c="k")
                else:
    ...
    ax_main.text(0.15, 0.95, f"Object: {object_name}", transform=ax_main.transAxes, fontsize=12,
...
# def plot_a_spectra(spectra,**kwargs):
#     x_axis,y_axis,yerr = spectra
#     xlim =  kwargs.get("xlim",[jnp.nanmin(x_axis),jnp.nanmax(x_axis)])
#     regions = {Halpha: [5600,7300]}
#     for key,values in regios.items()
    ####
#Halpha 5600,7300
#Hbeta 4400, 5600
#MII 2500, 3000
...
# chi = jnp.nansum(residual**2)
#     print("chi2_r",chi/(Halpha_class.n_gaussian*3+2 - len(Halpha_class.constrains_list)))
#     ax2.axhline(0,ls="--",linewidth=5,c="k")
#     ax2.scatter(x_axis, residual,alpha=0.9,zorder=10)
#     ax2.set_xlabel("wavelength A", fontsize=20)
#     ax2.set_ylabel("Normalized Residuals", fontsize=20)
...
#     else:
#         plt.show()
        #for i,line in enumerate(columns[:-2].reshape(3,Halpha_class.n_gaussian).T):


# def plot_n(n,xlim,save=None,**kwargs):
    
#     filtered_default_colors = [color for color in default_colors if color not in ['black', 'red']]*4
#     fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(35, 15),
#                                     gridspec_kw={'height_ratios': [2, 1]})
#     x_axis = region_to_fit_norm[n,0,:]
#     y_axis = region_to_fit_norm[n,1,:]
#     y_cont = Halpha_class.function_cont(x_axis,pandas_r[["cont_m","cont_b"]].iloc[n].values)
#     function_ind = GaussianSum(1)
#     fit_y =  function_all(x_axis,pandas_r.iloc[n].values)
#     yerr = uncertainties_region_to_fit_norm[n]
#     ylim = kwargs.get("ylim",[0,1.1])
#     #print( pandas_r.iloc[n])
#     verbose = kwargs.get("verbose",True)
#     for i,line in enumerate(columns[:-2].reshape(3,Halpha_class.n_gaussian).T):
#         values = pandas_r[line].iloc[n].values
#         c = "r"
#         if max(xlim)>values[1]>min(xlim):
#             keys = "_".join(pandas_r[line].iloc[n].keys()[0].split("_")[:2])
#             local_y = function_ind(x_axis,values) + y_cont
#             ext = ""
#             if values[0]<0.01:
#                 ext =f" amplitude found is {values[0]:.3f} less than 0.01"
#                 #print()
#             text = keys
#             if verbose:
#                 print("FWHM(km/s)",keys,wl_to_kms(values[2]*2.355,values[1]),f"sigma={values[2]}"+ext,f"center {values[1]}")
#             if "Halpha" in keys:
#                 c = "k"
#                 text = ""
#             ax1.plot(x_axis, local_y,zorder=3,ls='-.',color=filtered_default_colors[i])
