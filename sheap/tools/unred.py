...
def unred(wave, flux, ebv, R_V=3.1, LMC2=False, AVGLMC=False):
    ...
    XSPLOPIR = jnp.concatenate((jnp.array([0]),10000.0/jnp.array([26500.0,12200.0,6000.0,5470.0,4670.0,4110.0])))
    ysplir_   = jnp.array([0.0,0.26469,0.82925])*R_V/3.1
    ysplop_ = jnp.array([jnp.polyval(jnp.array([-4.22809e-01, 1.00270, 2.13572e-04][::-1]), R_V),
            jnp.polyval(jnp.array([-5.13540e-02, 1.00216, -7.35778e-05][::-1]), R_V),
            jnp.polyval(jnp.array([7.00127e-01, 1.00184, -3.32598e-05][::-1]), R_V),
            jnp.polyval(jnp.array([
                    1.19456, 1.01707, -5.46959e-03,
                    7.97809e-04, -4.45636e-05][::-1]), R_V)])
    ...
    yk, bk, ck, dk = cubic_spline_coefficients(jnp.concatenate((XSPLOPIR,xspluv)),jnp.concatenate((ysplopir_,yspluv_)))
