    ...
    def minimization_function(
        ...
        def optimize_model(initial_params, xs, y, y_uncertainties, constraints):
            ...
            if method == "lbfgs":
                ...
                def cond_fn(carry):
                    (_, state), i = carry
                    return (state.error >= optimizer.tolerance_grad) & (i < lbfgs_options.get("maxiter", 100))

                def body_fn(carry):
                    carry, loss = lbfgs_step(carry[0])
                    loss_history.append(loss)
                    return carry, carry[1] + 1

                (final_params, _), _ = lax.while_loop(
                    cond_fn,
                    body_fn,
                    ((initial_params, state), 0),
                )
                ...
                (final_params, _), loss_history = lax.scan(
                    step_fn, (initial_params, opt_state), None, length=1000
                )
